...
import { Button, Input, Link, Select, SelectItem, SelectedItems } from "@nextui-org/react";
...
import { Item, TransactionInBody, TransactionInInitialForm } from "./transactionInBody";
...
export default function TransactionInForm({ initialValues = initialForm, formType, initialValueOptions: { inventoryItems, initialItemValue = [{ key: "a7ga7w7ha-awhawh", id: "", price: 0, total: 1 }] }, onSubmit }: TransactionInFormProps) {
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="space-y-6">
        ...
        <div className="space-y-12">
          ...
          <AnimatePresence>
            {items.map((item, index) => (
              ...
              >
                <Select isRequired label="Barang" variant="bordered" labelPlacement="outside" placeholder="Pilih Barang" onChange={(e) => onItemChange("id", e.target.value, index)}>
          ...
          <div className="grid grid-cols-3 gap-10">
            <Input startContent={<RpIcon />} disabled className="col-start-3" name="totalPrice" type="text" variant="underlined" label="Total Harga" labelPlacement="outside-left" placeholder="Total Harga" value={totalPrice.toString()} />
          </div>
      ...
      <div className="pt-10 flex justify-end items-center gap-3">
        <Button as={Link} href="/dashboard/transactions-in" variant="light" className="font-semibold">
          Batal
        </Button>
