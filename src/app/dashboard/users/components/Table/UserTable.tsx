...
const INITIAL_VISIBLE_COLUMNS = ["namaLengkap", "username", "email", "role", "actions"];
...
export default function UserTable({ users }: UserTableProps) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((user) => user.namaLengkap.toLowerCase().includes(filterValue.toLowerCase()));
    }
    if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
      filteredUsers = filteredUsers.filter((user) => Array.from(statusFilter).includes(user.role));
    }
  ...
  const renderCell = React.useCallback((user: User, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      case "namaLengkap":
        return (
          <User avatarProps={{ radius: "lg", src: user.foto }} description={user.email} name={cellValue}>
            {user.email}
          </User>
        );
      case "role":
          ...
          <div className="relative flex justify-end items-center gap-2">
            <Dropdown>
              ...
              <DropdownMenu>
                ...
                <DropdownItem startContent={<FaRegTrashAlt />} color="danger" onClick={() => onDeleteItem(user.id)}>
                  Delete
                </DropdownItem>
              </DropdownMenu>
            </Dropdown>
          ...
          <div className="flex-1 flex justify-end gap-3">
            <Dropdown>
              <DropdownTrigger className="hidden sm:flex">
                <Button endContent={<IoChevronDownOutline className="text-small" />} variant="flat">
                  Status
                </Button>
              </DropdownTrigger>
            ...
            <Button as={Link} href="/dashboard/users/add" color="primary" className="font-semibold w-fit" startContent={<IoIosAdd size={20} />}>
              Karyawan
            </Button>
  ...
  }, [filterValue, statusFilter, visibleColumns, onSearchChange, onRowsPerPageChange, isLargeScreen, onClear]);

  const bottomContent = React.useMemo(() => {
    return (
      <div className="py-8 px-8 flex justify-end items-center bg-background rounded-b-large">
        <span className="w-[30%] text-small text-default-400">Total data: {users.length}</span>
